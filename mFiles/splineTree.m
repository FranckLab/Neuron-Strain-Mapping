function [t,n,b,K,T] = splineTree(swc,location,plot01)
%
...
for i = 1:nBR % number of branches
    ...
    if size(ri,1) > 3
        ...
        if strcmpi(location(1),'e')
            ...
            % valid vertex locations (end points not considered)
            BRi = BRi(2:end-1);
            t(BRi,:) = tangent(2:end-1,:);
            n(BRi,:) = normal(2:end-1,:);
            b(BRi,:) = binormal(2:end-1,:);
            K(BRi) = kappa(2:end-1,:);
            T(BRi) = tau(2:end-1,:);
        end
    end
...
% Find indices of nodes corresponding to segments that make up the neuron
